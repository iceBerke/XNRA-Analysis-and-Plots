...
from xnra_main_v1 import plot_xnra_spectrum
...
class XNRAViewerGUI:
    ...
    def create_general_tab(self):
        ...
        fig_frame.pack(fill="x", pady=(0, 10))
    ...
    def create_raw_data_tab(self):
        ...
        ttk.Checkbutton(scrollable_frame, text="Show Raw Data", variable=self.show_raw_data).pack(anchor=tk.W, pady=(0, 10))
        
        # Markers
        marker_frame = ttk.LabelFrame(scrollable_frame, text="Markers", padding="10")
        marker_frame.pack(fill="x", pady=(0, 10))
        ...
        ttk.Combobox(marker_frame, textvariable=self.raw_marker_shape, values=['circle', 'square', 'triangle'], state='readonly', width=15).grid(row=1, column=1, sticky=tk.W, padx=(5, 0), pady=(5, 0))
        
        ttk.Label(marker_frame, text="Fill:").grid(row=2, column=0, sticky=tk.W)
        self.raw_marker_fill = tk.StringVar(value='hollow')
        ttk.Combobox(marker_frame, textvariable=self.raw_marker_fill, values=['filled', 'hollow'], state='readonly', width=15).grid(row=2, column=1, sticky=tk.W, padx=(5, 0))
        ...
        ttk.Combobox(range_frame, textvariable=self.raw_marker_range_mode, values=['channel', 'energy'], state='readonly', width=15).grid(row=1, column=1, sticky=tk.W, padx=(5, 0), pady=(5, 0))
        ...
        self.raw_line_alpha = self.create_number_field(line_frame, "Transparency (0-1):", 3, 1.0)
    ...
    def create_smoothed_data_tab(self):
        ...
        ttk.Checkbutton(scrollable_frame, text="Show Smoothed Data", variable=self.show_smoothed_data).pack(anchor=tk.W, pady=(0, 10))
        ...
        ttk.Combobox(marker_frame, textvariable=self.smoothed_marker_shape, values=['circle', 'square', 'triangle'], state='readonly', width=15).grid(row=1, column=1, sticky=tk.W, padx=(5, 0), pady=(5, 0))
        
        ttk.Label(marker_frame, text="Fill:").grid(row=2, column=0, sticky=tk.W)
        self.smoothed_marker_fill = tk.StringVar(value='hollow')
        ttk.Combobox(marker_frame, textvariable=self.smoothed_marker_fill, values=['filled', 'hollow'], state='readonly', width=15).grid(row=2, column=1, sticky=tk.W, padx=(5, 0))
        ...
        ttk.Combobox(range_frame, textvariable=self.smoothed_marker_range_mode, values=['channel', 'energy'], state='readonly', width=15).grid(row=1, column=1, sticky=tk.W, padx=(5, 0), pady=(5, 0))
        ...
        self.smoothed_line_alpha = self.create_number_field(line_frame, "Transparency (0-1):", 3, 1.0)
    ...
    def create_appearance_tab(self):
        ...
        text_color_frame.pack(fill="x", pady=(0, 10))
        ...
        tick_frame.pack(fill="x", pady=(0, 10))
        ...
        ttk.Combobox(tick_frame, textvariable=self.tick_direction, values=['in', 'out', 'inout'], state='readonly', width=15).grid(row=3, column=1, sticky=tk.W, padx=(5, 0), pady=(5, 0))
    ...
    def create_axes_tab(self):
        ...
        ttk.Combobox(mode_frame, textvariable=self.x_axis_mode, values=['channel', 'energy'], state='readonly', width=15).grid(row=0, column=1, sticky=tk.W, padx=(5, 0))
    ...
    def create_number_field(self, parent, label, row, default, is_int=False):
        """Create a labeled number entry field"""
    ...
    def create_optional_number_field(self, parent, label, row):
        ...
        entry = ttk.Entry(parent, textvariable=var, width=15)
        entry.grid(row=row, column=1, sticky=tk.W, padx=(5, 0))
        ttk.Label(parent, text="(leave empty for auto)", font=('TkDefaultFont', 8, 'italic')).grid(row=row, column=2, sticky=tk.W, padx=(5, 0))
        return var
    ...
    def create_color_field(self, parent, label, row, default):
        ...
        color_preview = tk.Canvas(frame, width=20, height=20, bg=default, relief=tk.SUNKEN, borderwidth=1)
        ...
        ttk.Button(frame, text="Pick", command=pick_color, width=6).pack(side=tk.LEFT)
