...
# Differences from v5: added controls for the placement of the info and legend boxes
# Works with main v4 script
...
from xnra_main_v4 import plot_xnra_spectrum
...
class XNRAViewerGUI:
    ...
    def create_general_tab(self):
        ...
        ttk.Radiobutton(display_frame, text="Left", 
                    ...
                    value='left',
                    command=self.update_info_box_defaults).grid(row=3, column=0, sticky=tk.W, padx=(20, 0))
        ttk.Radiobutton(display_frame, text="Right", 
                    variable=self.info_box_position, 
                    value='right',
                    command=self.update_info_box_defaults).grid(row=4, column=0, sticky=tk.W, padx=(20, 0))
    
        ttk.Label(display_frame, text="Fine-tune Position:").grid(row=5, column=0, sticky=tk.W, pady=(10, 0))
        self.info_box_x_offset = self.create_number_field(display_frame, "X-offset:", 6, 0.05)
        self.info_box_y_offset = self.create_number_field(display_frame, "Y-offset:", 7, 0.95)
        ttk.Label(display_frame, text="(0.0 = left/bottom, 1.0 = right/top)", font=('TkDefaultFont', 7, 'italic')).grid(row=8, column=0, sticky=tk.W)
