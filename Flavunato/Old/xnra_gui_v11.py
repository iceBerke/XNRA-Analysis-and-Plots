...
try:
    from xnra_uncertainty_v6 import (
        parse_xnra_full,
...
class XNRAViewerGUI:
    ...
    def create_uncertainty_tab_v6(self):
        ...
        formula_frame = ttk.LabelFrame(left_panel, text="Uncertainty Formula", padding="10")
        formula_frame.grid(row=1, column=0, sticky="ew", pady=(0, 10))
        
        self.apply_chi2_adjustment = tk.BooleanVar(value=False)
        
        # Standard formula radio
        standard_frame = ttk.Frame(formula_frame)
        standard_frame.pack(fill=tk.X, pady=(0, 5))
        
        ttk.Radiobutton(standard_frame, text="Standard (Eq. 3)", 
                        variable=self.apply_chi2_adjustment, value=False,
                        command=self.update_formula_display).pack(side=tk.LEFT)
        
        ttk.Label(standard_frame, text="ΔC = √[(ΔQ·Ω)² + (Δσ)² + (1/√N)²]",
                  font=('Consolas', 9), foreground='gray').pack(side=tk.LEFT, padx=(10, 0))
        
        # χ²-adjusted formula radio
        adjusted_frame = ttk.Frame(formula_frame)
        adjusted_frame.pack(fill=tk.X, pady=(5, 0))
        
        ttk.Radiobutton(adjusted_frame, text="χ²-Adjusted (Eq. 6)", 
                        variable=self.apply_chi2_adjustment, value=True,
                        command=self.update_formula_display).pack(side=tk.LEFT)
        
        ttk.Label(adjusted_frame, text="ΔC = √[(ΔQ·Ω)² + (Δσ)² + (√χ²/ν · 1/√N)²]",
                  font=('Consolas', 9), foreground='gray').pack(side=tk.LEFT, padx=(10, 0))
        
        # Explanation
        self.formula_explanation = tk.StringVar(value="Using standard formula (no χ² adjustment)")
        ...
        chi2_info = (
            "χ²/ν ≈ 1.0  →  Good fit, uncertainties reliable\n"
            "χ²/ν > 1.0  →  May indicate underestimated uncertainties\n"
            "                   (consider using χ²-adjusted formula)\n"
            "χ²/ν < 1.0  →  May indicate overestimated uncertainties"
        )
        ttk.Label(chi2_frame, text=chi2_info, font=('TkDefaultFont', 8), justify=tk.LEFT).pack(anchor=tk.W)
        
        # ===== ROI DEFINITION =====
        roi_frame = ttk.LabelFrame(left_panel, text="Regions of Interest (ROIs)", padding="10")
        roi_frame.grid(row=3, column=0, sticky="nsew", pady=(0, 10))
        ...
        calc_frame.grid(row=4, column=0, sticky="ew")
        ...
        # Results treeview with χ² factor column
        columns = ('Layer', 'Element', 'Conc (%)', '± Stat', '± Q·Ω', '± σ', '± Total', 'ROI', 'χ²/ν', '×√χ²')
        self.results_tree = ttk.Treeview(right_panel, columns=columns, show='headings', height=20)
        
        col_widths = {
            'Layer': 42, 'Element': 55, 'Conc (%)': 65, 
            '± Stat': 55, '± Q·Ω': 50, '± σ': 40, '± Total': 60,
            'ROI': 65, 'χ²/ν': 55, '×√χ²': 45
        }
    ...
    def update_formula_display(self):
        ...
        if self.apply_chi2_adjustment.get():
            self.formula_explanation.set(
                "Using χ²-adjusted formula: statistical uncertainty scaled by √χ²/ν\n"
                "(Recommended when χ²/ν > 1 to account for underestimated uncertainties)"
            )
        else:
            self.formula_explanation.set(
                "Using standard formula (no χ² adjustment)"
            )
    ...
    def calculate_uncertainties_v6(self):
        ...
        try:
            ...
            reaction = self.selected_reaction.get()
            use_chi2_adj = self.apply_chi2_adjustment.get()
                ...
                for layer in data['layers']:
                    for element in roi_elements:
                        ...
                        unc = calculate_uncertainty_v6(
                            ...
                            xsec_unc,
                            chi2_reduced=chi2_r,
                            apply_chi2_adjustment=use_chi2_adj
                        )
                        ...
                        if use_chi2_adj and chi2_factor > 1.0:
                            factor_str = f"×{chi2_factor:.2f}"
                        else:
                            factor_str = "—"
                        ...
                        results.append({
                            ...
                            'sigma_stat_adjusted_percent': unc['sigma_stat_adjusted'] * 100 if not np.isnan(unc['sigma_stat_adjusted']) else None,
                            ...
                            'chi2_adjustment_applied': use_chi2_adj,
            ...
            formula_str = "χ²-adjusted" if use_chi2_adj else "Standard"
            self.uncertainty_status.config(
                text=f"✓ {len(results)} results | {formula_str} formula | "
                     f"Q·Ω: {qomega_unc*100:.1f}%, σ: {xsec_unc*100:.1f}%"
            )
    ...
    def export_depth_profile(self):
        ...
        try:
            ...
            use_adj = self.apply_chi2_adjustment.get()
            profile = create_depth_profile(df, use_adjusted=use_adj)
            ...
            if filename.endswith('.xlsx'):
                with pd.ExcelWriter(filename, engine='openpyxl') as writer:
                    ...
                    summary = pd.DataFrame([{
                        ...
                        'Parameter': 'χ² Adjustment Applied',
                        'Value': 'Yes' if use_adj else 'No'
                    }, {
                        'Parameter': 'Formula',
                        'Value': 'Eq. 6 (χ²-adjusted)' if use_adj else 'Eq. 3 (Standard)'
                    }])
        ...
        if filename:
            try:
                preset = {
                    ...
                    'apply_chi2_adjustment': self.apply_chi2_adjustment.get(),
                }
    ...
    def load_preset(self):
        ...
        if filename:
            try:
                ...
                if 'apply_chi2_adjustment' in preset:
                    self.apply_chi2_adjustment.set(preset['apply_chi2_adjustment'])
