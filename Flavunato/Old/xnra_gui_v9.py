...
# VERSION 4: Added Uncertainty Calculation tab

# Compatible with main v6 (hopefully, includes simulated data)
...
# Import uncertainty calculation
try:
    ...
    print("Warning: xnra_uncertainty_v4 not found. Uncertainty calculations disabled.")


class XNRAViewerGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("XNRA Spectrum Viewer - v4")
    ...
    def create_tabs(self, parent):
        ...
        self.create_uncertainty_tab()  # NEW TAB!
    ...
    def create_general_tab(self):
        ...
        scrollable_frame.columnconfigure(1, weight=2)  # for larger areas that occupy a single row
        
        # STACK container (on the left)
        stack = ttk.Frame(scrollable_frame)
        stack.grid(row=0, column=0, sticky="nsew", padx=(8, 0), pady=(0, 10))
        stack.columnconfigure(0, weight=1) # for horizontal stretching

        # Small section 1 (top of stack)
        ...
        title_entry = ttk.Entry(title_frame, textvariable=self.custom_title, width=50)
        ...
        # Figure settings
        # Small section 2 (bottom of stack)
        ...
        ttk.Radiobutton(display_frame, text="Left", 
                        variable=self.legend_position, 
                        value='left',
                        command=self.update_legend_defaults).grid(row=2, column=0, sticky=tk.W, padx=(20, 0))
        ttk.Radiobutton(display_frame, text="Right", 
                        variable=self.legend_position, 
                        value='right',
        ...
        # Visual separator
        ttk.Separator(display_frame, orient='horizontal').grid(row=8, column=0, columnspan=2, 
        ...
        ttk.Radiobutton(display_frame, text="Left", 
                        variable=self.info_box_position, 
                        value='left',
    ...
    def update_info_box_defaults(self):
        """Update offset defaults when position radio button changes"""
    ...
    def update_legend_defaults(self):
        """Update offset defaults when legend position radio button changes"""
    ...
    def create_simulated_data_tab(self):
        """Simulated data settings tab - NEW!"""
        tab = ttk.Frame(self.notebook, padding="10")
    ...
    # =========================================================================
    # NEW: UNCERTAINTY CALCULATION TAB
    # =========================================================================
    
    def create_uncertainty_tab(self):
        """Uncertainty calculation tab"""
        tab = ttk.Frame(self.notebook, padding="10")
        self.notebook.add(tab, text="Uncertainty")
        
        # Main layout: Left panel (settings) + Right panel (results)
        ...
        # Technique selection
        tech_frame = ttk.LabelFrame(left_panel, text="Technique", padding="10")
        ...
        # Systematic uncertainty info
        self.sys_unc_label = ttk.Label(tech_frame, text="Systematic σ: 3%", font=('TkDefaultFont', 9, 'italic'))
        self.sys_unc_label.grid(row=0, column=2, sticky=tk.W, padx=(20, 0))
        
        # Update display when technique changes
        def update_sys_unc(*args):
            tech = self.uncertainty_technique.get()
            sys_unc = {'RBS': 3, 'ERDA': 5, 'NRA': 5}.get(tech, 5)
            self.sys_unc_label.config(text=f"Systematic σ: {sys_unc}%")
        self.uncertainty_technique.trace('w', update_sys_unc)
        
        # ROI definition section
        roi_frame = ttk.LabelFrame(left_panel, text="Regions of Interest (ROIs)", padding="10")
        roi_frame.grid(row=1, column=0, sticky="nsew", pady=(0, 10))
        ...
        ttk.Label(header_frame, text="Elements (comma-separated)", width=25).pack(side=tk.LEFT, padx=2)
        
        # Scrollable ROI list
        roi_canvas = tk.Canvas(roi_frame, height=200)
        ...
        ttk.Button(calc_frame, text="Calculate Uncertainties", 
                   command=self.calculate_uncertainties).pack(side=tk.LEFT, padx=(0, 10))
        ttk.Button(calc_frame, text="Export Results", 
                   command=self.export_uncertainty_results).pack(side=tk.LEFT)
        ...
        for col in columns:
            ...
            width = 80 if col not in ('Element', 'ROI') else 60
        ...
        # Status label
        self.uncertainty_status = ttk.Label(right_panel, text="Load a file and define ROIs to calculate uncertainties", 
    ...
    def add_roi_row(self, name="", start="", end="", elements=""):
        ...
        ttk.Entry(row_frame, textvariable=elements_var, width=25).pack(side=tk.LEFT, padx=2)
    ...
    def calculate_uncertainties(self):
        ...
        if not UNCERTAINTY_AVAILABLE:
            messagebox.showerror("Error", "Uncertainty calculation module not available.\n\nMake sure xnra_uncertainty_v4.py is in the same directory.")
            return
        ...
        try:
            ...
            for roi in rois:
                ...
                for layer in data['layers']:
                    for element in roi_elements:
                        ...
                        unc = calculate_uncertainty_for_element(concentration, roi_counts, technique)
                        ...
                        results.append({
                            ...
                            'sigma_stat_percent': unc['sigma_stat'] * 100,
            ...
            self.uncertainty_status.config(text=f"✓ Calculated {len(results)} results from {len(rois)} ROI(s)")
    ...
    def export_uncertainty_results(self):
        """Export uncertainty results to Excel or CSV"""
    ...
    def get_current_settings(self):
        ...
        return {
            ...
            # Uncertainty
            'uncertainty_technique': self.uncertainty_technique.get(),
        }
    ...
    def apply_settings(self, settings):
        ...
        if 'custom_title' in settings: self.custom_title.set(settings['custom_title'])  
        ...
        if 'legend_position' in settings: self.legend_position.set(settings['legend_position'])      
        if 'legend_x_offset' in settings: self.legend_x_offset.set(settings['legend_x_offset'])      
        if 'legend_y_offset' in settings: self.legend_y_offset.set(settings['legend_y_offset'])      
        if 'info_box_position' in settings: self.info_box_position.set(settings['info_box_position'])
        if 'info_box_x_offset' in settings: self.info_box_x_offset.set(settings['info_box_x_offset'])  
        if 'info_box_y_offset' in settings: self.info_box_y_offset.set(settings['info_box_y_offset'])  
        # Raw data
        ...
        # Simulated data
        if 'show_simulated_data' in settings: self.show_simulated_data.set(settings['show_simulated_data'])
        if 'show_simulated_markers' in settings: self.show_simulated_markers.set(settings['show_simulated_markers'])
        if 'simulated_marker_shape' in settings: self.simulated_marker_shape.set(settings['simulated_marker_shape'])
        if 'simulated_marker_fill' in settings: self.simulated_marker_fill.set(settings['simulated_marker_fill'])
        if 'simulated_marker_color' in settings: self.simulated_marker_color.set(settings['simulated_marker_color'])
        if 'simulated_marker_size' in settings: self.simulated_marker_size.set(settings['simulated_marker_size'])
        if 'simulated_marker_edge_width' in settings: self.simulated_marker_edge_width.set(settings['simulated_marker_edge_width'])
        if 'simulated_marker_range_enabled' in settings: self.simulated_marker_range_enabled.set(settings['simulated_marker_range_enabled'])
        if 'simulated_marker_range_mode' in settings: self.simulated_marker_range_mode.set(settings['simulated_marker_range_mode'])
        if 'simulated_marker_range_min' in settings: self.simulated_marker_range_min.set(settings['simulated_marker_range_min'])
        if 'simulated_marker_range_max' in settings: self.simulated_marker_range_max.set(settings['simulated_marker_range_max'])
        if 'show_simulated_line' in settings: self.show_simulated_line.set(settings['show_simulated_line'])
        if 'simulated_line_color' in settings: self.simulated_line_color.set(settings['simulated_line_color'])
        if 'simulated_line_width' in settings: self.simulated_line_width.set(settings['simulated_line_width'])
        if 'simulated_line_alpha' in settings: self.simulated_line_alpha.set(settings['simulated_line_alpha'])
        # Appearance
        ...
        # Uncertainty
        if 'uncertainty_technique' in settings: self.uncertainty_technique.set(settings['uncertainty_technique'])
