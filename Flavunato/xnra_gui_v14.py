### XNRA Spectrum Viewer GUI - Version 6 ###
# Tkinter-based graphical interface for XNRA spectrum visualization
# VERSION 6: Implements French IBA expert uncertainty formulas
#            - Separate Q·Ω (charge × solid angle) input
#            - Cross-section dropdown with reaction-specific values
#            - Statistical uncertainty from ROI counts
#            - Optional χ²-adjusted uncertainty calculation

# Compatible with xnra_main_v6 and xnra_uncertainty_v6
...
# Import uncertainty calculation (v6 with French expert formulas)
try:
    from xnra_uncertainty_v8 import (
        parse_xnra_full,
        calculate_chi_squared_for_roi,
        calculate_uncertainty_v6,
    ...
    print("Warning: xnra_uncertainty_v6 not found. Uncertainty calculations disabled.")


class XNRAViewerGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("XNRA Spectrum Viewer - v6")
    ...
    def create_tabs(self, parent):
        ...
        self.create_uncertainty_tab_v6()
    ...
    # =========================================================================
    # UNCERTAINTY TAB v6 - French Expert Formulas with χ² Adjustment
    # =========================================================================
    
    def create_uncertainty_tab_v6(self):
        """
        Uncertainty calculation tab implementing French IBA expert formulas:
        
        Standard (Eq. 3):  ΔC_A = √[(ΔQΩ)² + (Δσ_A)² + (ΔA_A)²]
        Adjusted (Eq. 6):  ΔC_A = √[(ΔQΩ)² + (Δσ_A)² + (√χ²/ν × ΔA_A)²]
        """
        tab = ttk.Frame(self.notebook, padding="10")
        self.notebook.add(tab, text="Uncertainty (v6)")
        
        tab.columnconfigure(0, weight=1)
        tab.columnconfigure(1, weight=1)
        tab.rowconfigure(0, weight=1)
        
        # ===== LEFT PANEL: Settings (with scrolling) =====
        ...
        # ===== UNCERTAINTY INPUTS FRAME =====
        inputs_frame = ttk.LabelFrame(left_panel, text="Uncertainty Components", padding="10")
        ...
        qomega_entry.grid(row=0, column=1, sticky=tk.W)
        
        qomega_info = ttk.Label(inputs_frame, 
                                text="Std. dev. of (simulated / measured) Particle·sr ratios",
                                font=('TkDefaultFont', 8, 'italic'), foreground='gray')
        qomega_info.grid(row=1, column=0, columnspan=2, sticky=tk.W, pady=(0, 10))
        
        # 2. Cross-section reaction dropdown
        ttk.Label(inputs_frame, text="Reaction:", 
        ...
        ttk.Separator(inputs_frame, orient='horizontal').grid(row=4, column=0, columnspan=2, 
                                                               sticky='ew', pady=10)
        
        # 3. Statistical uncertainty info
        stat_label = ttk.Label(inputs_frame, text="Statistical (Poisson):", 
                               font=('TkDefaultFont', 9, 'bold'))
        stat_label.grid(row=5, column=0, sticky=tk.W)
        ttk.Label(inputs_frame, text="Calculated from ROI counts",
                  font=('TkDefaultFont', 8, 'italic'), foreground='gray').grid(row=5, column=1, sticky=tk.W)
        
        ttk.Label(inputs_frame, text="Formula: ΔA_A = 1/√N  (N = element counts in ROI)",
                  font=('TkDefaultFont', 8)).grid(row=6, column=0, columnspan=2, sticky=tk.W, pady=(2, 0))
        
        # ===== FORMULA SELECTION FRAME =====
        ...
        # Standard formula radio (no adjustment)
        ...
        # ===== CHI-SQUARED INFO =====
        chi2_frame = ttk.LabelFrame(left_panel, text="χ²/ν Interpretation", padding="10")
        chi2_frame.grid(row=2, column=0, sticky="ew", pady=(0, 10))
        ...
        ttk.Label(chi2_frame, text=chi2_info, font=('Consolas', 8), justify=tk.LEFT).pack(anchor=tk.W)
        
        # ===== ELEMENT SPECTRUM (for accurate A_A calculation) =====
        elem_spec_frame = ttk.LabelFrame(left_panel, text="Element Spectrum (Recommended)", padding="10")
        elem_spec_frame.grid(row=3, column=0, sticky="ew", pady=(0, 10))
        
        elem_spec_info = ttk.Label(elem_spec_frame, 
            text="Load SIMNRA element spectrum for accurate statistical uncertainty.\n"
                 "Export via: Plot → Show Element Spectra, then File → Write Spectrum Data",
            font=('TkDefaultFont', 8), foreground='gray', justify=tk.LEFT)
        elem_spec_info.pack(anchor=tk.W, pady=(0, 5))
        
        elem_file_frame = ttk.Frame(elem_spec_frame)
        elem_file_frame.pack(fill=tk.X)
        
        ttk.Button(elem_file_frame, text="Load Element Spectrum...", 
                   command=self.load_element_spectrum).pack(side=tk.LEFT, padx=(0, 5))
        ttk.Button(elem_file_frame, text="Clear", 
                   command=self.clear_element_spectrum).pack(side=tk.LEFT, padx=5)
        
        self.elem_spec_status = tk.StringVar(value="⚠ Not loaded (using approximate method)")
        ...
        self.elem_spec_status_label.pack(anchor=tk.W, pady=(5, 0))
        ...
        ttk.Button(calc_frame, text="Calculate Uncertainties", 
                   command=self.calculate_uncertainties_v6).pack(side=tk.LEFT, padx=(0, 5))
        ...
        # Results treeview with χ², AUC, and element fraction columns
        columns = ('Layer', 'Element', 'Conc (%)', '± Stat', '± Q·Ω', '± σ', '± Total', 'ROI', 'A_A', 'f_A', 'χ²/ν', 'Status', 'AUC')
        self.results_tree = ttk.Treeview(right_panel, columns=columns, show='headings', height=20)
        
        col_widths = {
            'Layer': 42, 'Element': 55, 'Conc (%)': 60, 
            '± Stat': 52, '± Q·Ω': 48, '± σ': 38, '± Total': 56,
    ...
    def load_element_spectrum(self):
        """Load SIMNRA element spectrum file for accurate A_A calculation"""
        filename = filedialog.askopenfilename(
            title="Load Element Spectrum",
            filetypes=[
                ("Text files", "*.txt"),
                ("Data files", "*.dat"),
                ("All files", "*.*")
        ...
        try:
            ...
            elements = get_available_elements(self.element_spectrum_data)
            n_channels = len(self.element_spectrum_data['channels'])
            
            self.elem_spec_status.set(
                f"✓ Loaded: {Path(filename).name}\n"
                f"   {len(elements)} elements: {', '.join(elements[:8])}{'...' if len(elements) > 8 else ''}\n"
                f"   {n_channels} channels"
            )
            self.elem_spec_status_label.config(foreground='#059669')  # Green
            ...
            self.elem_spec_status_label.config(foreground='#B45309')  # Orange
    
    def clear_element_spectrum(self):
        """Clear loaded element spectrum"""
        self.element_spectrum_data = None
        self.element_spectrum_file.set("")
        self.elem_spec_status.set("⚠ Not loaded (using approximate method)")
        self.elem_spec_status_label.config(foreground='#B45309')  # Orange
    
    def calculate_uncertainties_v6(self):
        """Calculate uncertainties using v6 formulas (French IBA expert)"""
        if not UNCERTAINTY_AVAILABLE:
            messagebox.showerror("Error", 
                "Uncertainty calculation module not available.\n\n"
                "Make sure xnra_uncertainty_v6.py is in the same directory.")
            return
        ...
        try:
            ...
            for roi in rois:
                ...
                for layer in data['layers']:
                    for element in roi_elements:
                        ...
                        # Calculate element counts using simulation-guided method if available
                        element_fraction = None
                        if self.element_spectrum_data is not None and data['raw_counts'] is not None:
                            # Use physically accurate method with element spectrum
                            ...
                            # Fallback to approximate method (roi_counts × concentration)
                            element_counts = int(roi_counts * concentration)
                            element_fraction = concentration  # Approximate
                        ...
                        values = (
                            ...
                            fmt_unc(unc['sigma_stat']),
                            fmt_unc(unc['sigma_qomega']),
                            fmt_unc(unc['sigma_xsec']),
                            fmt_unc(unc['sigma_total']),
                            roi_name,
                            aa_display,
                            fa_display,
                            chi2_display,
            ...
            self.uncertainty_status.config(
                text=f"✓ {len(results)} results | {method_names.get(adj_method, adj_method)} | "
                     f"Q·Ω: {qomega_unc*100:.1f}%, σ: {xsec_unc*100:.1f}% | {aa_method_str}"
            )
    ...
    def export_uncertainty_results(self):
        ...
        try:
            ...
            if filename.endswith('.xlsx'):
                df.to_excel(filename, index=False, sheet_name='Uncertainties_v6')
            else:
    ...
    def save_preset(self):
        ...
        if filename:
            try:
                preset = {
                    'qomega_uncertainty': self.qomega_uncertainty.get(),
                    'selected_reaction': self.selected_reaction.get(),
                    'xsec_uncertainty': self.xsec_uncertainty.get(),
                    'adjustment_method': self.adjustment_method.get(),
                }
        ...
        if filename:
            try:
                with open(filename, 'r') as f:
                    preset = json.load(f)
                if 'qomega_uncertainty' in preset:
                    self.qomega_uncertainty.set(preset['qomega_uncertainty'])
                if 'selected_reaction' in preset:
                    self.selected_reaction.set(preset['selected_reaction'])
                    self.on_reaction_changed()
                if 'xsec_uncertainty' in preset:
                    self.xsec_uncertainty.set(preset['xsec_uncertainty'])
                if 'adjustment_method' in preset:
                    self.adjustment_method.set(preset['adjustment_method'])
                    self.update_formula_display()
                # Backward compatibility with old presets
                elif 'apply_chi2_adjustment' in preset:
                    if preset['apply_chi2_adjustment']:
                        self.adjustment_method.set('chi2')
                    else:
                        self.adjustment_method.set('none')
                    self.update_formula_display()
