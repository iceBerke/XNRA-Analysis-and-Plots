### XNRA Spectrum Viewer GUI - Version 18 ###
...
class XNRAViewerGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("XNRA Spectrum Viewer - v18 (last update: 09/04/2026)")
    ...
    def create_element_spectrum_tab(self):
        ...
        self.show_total_spectrum = tk.BooleanVar(value=True)
        ttk.Checkbutton(options_frame, text="Show Total Spectrum (dashed)", 
                        variable=self.show_total_spectrum).grid(row=0, column=0, sticky=tk.W)
        
        self.elem_x_axis_mode = tk.StringVar(value='channel')
        ttk.Label(options_frame, text="X-axis:").grid(row=0, column=1, sticky=tk.E, padx=(20, 5))
        ttk.Radiobutton(options_frame, text="Channel", variable=self.elem_x_axis_mode, 
                        value='channel').grid(row=0, column=2, sticky=tk.W)
        ttk.Radiobutton(options_frame, text="Energy", variable=self.elem_x_axis_mode, 
                        value='energy').grid(row=0, column=3, sticky=tk.W)
        
        # Energy calibration (from .xnra file - read-only)
        cal_frame = ttk.Frame(options_frame)
        cal_frame.grid(row=1, column=0, columnspan=4, sticky=tk.W, pady=(5, 0))
        
        ttk.Label(cal_frame, text="Energy Cal (from .xnra):").pack(side=tk.LEFT)
        ttk.Label(cal_frame, text="Offset:").pack(side=tk.LEFT, padx=(10, 2))
        self.elem_cal_offset = tk.DoubleVar(value=0.0)
        offset_entry = ttk.Entry(cal_frame, textvariable=self.elem_cal_offset, width=8, state='readonly')
        offset_entry.pack(side=tk.LEFT)
        ttk.Label(cal_frame, text="keV").pack(side=tk.LEFT, padx=(2, 10))
        ttk.Label(cal_frame, text="Gain:").pack(side=tk.LEFT, padx=(0, 2))
        self.elem_cal_gain = tk.DoubleVar(value=1.0)
        gain_entry = ttk.Entry(cal_frame, textvariable=self.elem_cal_gain, width=8, state='readonly')
        gain_entry.pack(side=tk.LEFT)
        ttk.Label(cal_frame, text="keV/ch").pack(side=tk.LEFT, padx=(2, 0))
        
        cal_note = ttk.Label(options_frame, 
            text="💡 Load .xnra file (top) to get calibration for energy axis",
            font=('TkDefaultFont', 8, 'italic'), foreground='#666')
        cal_note.grid(row=2, column=0, columnspan=4, sticky=tk.W, pady=(2, 5))
        
        # Plot title
        title_frame = ttk.Frame(options_frame)
        title_frame.grid(row=3, column=0, columnspan=4, sticky=tk.W, pady=(5, 0))
    ...
    def create_uncertainty_tab_v9(self):
        ...
        """
        tab = ttk.Frame(self.notebook, padding="10")
        self.notebook.add(tab, text="Uncertainty calc")
        ...
        # ===== ISOTOPE MODE FRAME =====
        isotope_frame = ttk.LabelFrame(left_panel, text="Isotope Mode", padding="10")
        isotope_frame.grid(row=1, column=0, sticky="ew", pady=(0, 10))
        
        self.isotope_mode = tk.BooleanVar(value=False)
        ttk.Checkbutton(isotope_frame, text="Enable isotope-specific calculations", 
                        variable=self.isotope_mode).pack(anchor=tk.W)
        
        ttk.Label(isotope_frame, 
                  text="When enabled, isotopes (e.g., ¹²C, ¹⁶O) are calculated separately\n"
                       "with their own cross-section uncertainties.",
                  font=('TkDefaultFont', 8, 'italic'), foreground='gray').pack(anchor=tk.W, pady=(5, 0))
        
        # Isotope overrides section
        iso_override_frame = ttk.Frame(isotope_frame)
        iso_override_frame.pack(fill=tk.X, pady=(10, 0))
        
        ttk.Button(iso_override_frame, text="Add Isotope Override", 
                   command=self.add_isotope_override_row).pack(side=tk.LEFT, padx=(0, 5))
        ttk.Button(iso_override_frame, text="Clear Overrides", 
                   command=self.clear_isotope_overrides).pack(side=tk.LEFT)
        
        self.isotope_override_container = ttk.Frame(isotope_frame)
        self.isotope_override_container.pack(fill=tk.X, pady=(5, 0))
        ...
        # ===== ELEMENT SPECTRUM STATUS (file loaded in Element/Iso Data tab) =====
        ...
        columns = ('Layer', 'Element', 'Conc(%)', '±Stat', 'Q·Ω(rel)', 'σ(rel)', '±Total', 'ROI', 'A_A', 'f_A', 'χ²/ν', 'Status', 'AUC')
        self.results_tree = ttk.Treeview(right_panel, columns=columns, show='headings', height=20)
        
        col_widths = {
            'Layer': 42, 'Element': 60, 'Conc(%)': 55, 
            '±Stat': 50, 'Q·Ω(rel)': 58, 'σ(rel)': 48, '±Total': 52,
        ...
        legend_text = "Q·Ω(rel) and σ(rel) are CONSTANT relative uncertainties (%). ±Stat and ±Total are ABSOLUTE (% of sample)."
    ...
    # ===== Element/Iso Data Viewer Methods =====
    ...
    def plot_element_spectra_gui(self):
        ...
        # Check for .xnra file if energy mode selected
        ...
        if not elements:
            messagebox.showwarning("Warning", "Please select at least one element to plot!")
            return
        ...
        try:
            fig = plot_element_spectra(
                ...
                show_total=self.show_total_spectrum.get(),
    ...
    def plot_isotope_spectra_gui(self):
        ...
        if not isotopes:
            messagebox.showwarning("Warning", "Please select at least one isotope to plot!")
            return
    ...
    def plot_all_selected_spectra(self):
        ...
        if not elements and not isotopes:
            messagebox.showwarning("Warning", "Please select at least one element or isotope to plot!")
            return
            ...
            for roi in rois:
                ...
                for layer in data['layers']:
                    for element_or_isotope in roi_elements:
                        ...
                        def fmt_unc_abs(val):
                            """Format ABSOLUTE uncertainty (concentration-scaled)"""
                            if val is None or (isinstance(val, float) and np.isnan(val)):
                                return "N/A"
                            return f"{val*100:.3f}"
                        
                        def fmt_unc_rel(val):
                            """Format RELATIVE uncertainty (constant)"""
                            if val is None or (isinstance(val, float) and np.isnan(val)):
                                return "N/A"
                            return f"{val*100:.2f}"
                        ...
                        values = (
                            ...
                            fmt_unc_abs(unc['sigma_stat']),      # Stat: ABSOLUTE (varies)
    ...
    def browse_file(self):
        ...
        if filename:
            ...
            # Extract calibration for Element/Iso Data tab
            try:
                ...
                # If parsing fails, just set defaults
        ...
        if filename:
            try:
                preset = {
                    ...
                    'isotope_mode': self.isotope_mode.get(),
                    'isotope_overrides': {w['isotope'].get(): w['uncertainty'].get() 
                                          for w in self.isotope_override_widgets 
                                          if w['isotope'].get() and w['uncertainty'].get()},
                }
    ...
    def load_preset(self):
        ...
        if filename:
            try:
                ...
                if 'isotope_mode' in preset:
                    self.isotope_mode.set(preset['isotope_mode'])
                if 'isotope_overrides' in preset:
                    self.clear_isotope_overrides()
                    for iso, unc in preset['isotope_overrides'].items():
                        self.add_isotope_override_row(iso, unc)
