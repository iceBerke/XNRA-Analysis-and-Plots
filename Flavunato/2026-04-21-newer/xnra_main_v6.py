...
# Difference from version 5: Added simulated/calculated data support
...
def parse_xnra_file(filepath):
    ...
    # Particles × solid angle (Q·Ω). SIMNRA XNRA uses <beamfluence> for
    # the particle count and <solidangle units="msr"> for Ω (note: millisteradians).
    fluence_node = root.find('.//idf:beamfluence', ns)
    data['particles'] = float(fluence_node.text) if fluence_node is not None and fluence_node.text else None

    sa_node = root.find('.//idf:solidangle', ns)
    if sa_node is not None and sa_node.text:
        sa_value = float(sa_node.text)
        sa_units = (sa_node.get('units') or 'sr').lower()
        # Convert to sr if needed
        if sa_units == 'msr':
            sa_value *= 1e-3
        data['solid_angle_sr'] = sa_value
    else:
        data['solid_angle_sr'] = None

    if data['particles'] is not None and data['solid_angle_sr'] is not None:
        data['particles_sr'] = data['particles'] * data['solid_angle_sr']
    else:
        data['particles_sr'] = None
    ...
    # Try multiple common paths for simulated data
    simulated_data = None
    
    # Primary path (your structure)
    simulated_data = root.find('.//idf:simulation[1]/idf:simpledata', ns)

    # Fallbacks for compatibility
    if simulated_data is None:
        simulated_data = root.find('.//simnra:calculateddata/idf:simpledata', ns)

    if simulated_data is not None:
        x_text = simulated_data.find('idf:x', ns).text
        y_text = simulated_data.find('idf:y', ns).text

        data['simulated_channels'] = np.array([float(x) for x in x_text.split()])
        data['simulated_counts'] = np.array([float(y) for y in y_text.split()])

        print(f"✓ Simulated data: {len(data['simulated_channels'])} channels")
    else:
        data['simulated_channels'] = None
        data['simulated_counts'] = None
        print("ℹ️  No simulated data found")
...
def plot_xnra_spectrum(
    ...
    show_simulated_markers=False,
    simulated_marker_shape='circle',
    simulated_marker_fill='filled',
    simulated_marker_color='green',
    simulated_marker_size=2,
    simulated_marker_edge_width=1,
    simulated_marker_range_enabled=False,
    simulated_marker_range_mode='channel',
    simulated_marker_range_min=0,
    simulated_marker_range_max=1000,
    show_simulated_line=True,
    simulated_line_color='green',
    simulated_line_width=2.0,
    simulated_line_alpha=0.8,
    # Plot appearance
    ...
    if data['raw_counts'] is None and data['smoothed_counts'] is None and data['simulated_counts'] is None:
    ...
    simulated_channels = data['simulated_channels']
    simulated_counts = data['simulated_counts']
    ...
    if show_simulated_data and simulated_channels is not None and simulated_counts is not None:
        marker, fillstyle = get_marker_style(simulated_marker_shape, simulated_marker_fill)
        
        if show_simulated_line:
            ax.plot(simulated_channels, simulated_counts, 
                    linestyle='-',
                    linewidth=simulated_line_width,
                    color=simulated_line_color,
                    alpha=simulated_line_alpha,
                    label='Simulated data',
                    zorder=2)
            print("✓ Plotted simulated data line")
        
        if show_simulated_markers:
            marker_channels, marker_counts = apply_marker_range(
                simulated_channels, simulated_counts,
                simulated_marker_range_enabled,
                simulated_marker_range_mode,
                simulated_marker_range_min,
                simulated_marker_range_max,
                cal_offset,
                cal_gain
            )
            
            if marker_channels is not None:
                if simulated_marker_fill.lower() == 'hollow':
                    ax.plot(marker_channels, marker_counts,
                           linestyle='',
                           marker=marker,
                           markersize=simulated_marker_size,
                           markerfacecolor='none',
                           markeredgecolor=simulated_marker_color,
                           markeredgewidth=simulated_marker_edge_width,
                           alpha=simulated_line_alpha,
                           label='Simulated data' if not show_simulated_line else '',
                           zorder=3)
                else:
                    ax.plot(marker_channels, marker_counts,
                           linestyle='',
                           marker=marker,
                           markersize=simulated_marker_size,
                           markerfacecolor=simulated_marker_color,
                           markeredgecolor=simulated_marker_color,
                           alpha=simulated_line_alpha,
                           label='Simulated data' if not show_simulated_line else '',
                           zorder=3)
                
                if simulated_marker_range_enabled:
                    unit = 'keV' if simulated_marker_range_mode.lower() == 'energy' else 'channels'
                    print(f"✓ Plotted simulated markers: {simulated_marker_shape}, {simulated_marker_fill} ({simulated_marker_range_min}-{simulated_marker_range_max} {unit})")
            else:
                unit = 'keV' if simulated_marker_range_mode.lower() == 'energy' else 'channels'
                print(f"⚠️  No simulated data points in range {simulated_marker_range_min}-{simulated_marker_range_max} {unit}")
    ...
    # 2-decimal tick labels on primary x (channel) and y (counts) axes
    ...
    if show_legend:
        ...
        x_legend_pos = legend_x_offset 
        y_legend_pos = legend_y_offset
        ...
        legend = ax.legend(by_label.values(), by_label.keys(),
                      loc=loc_pos,
                      bbox_to_anchor=(x_legend_pos, y_legend_pos),
    ...
    if show_info_box:
        ...
        if data.get('particles_sr') is not None:
            info_lines.append(f"Particles·sr: {data['particles_sr']:.3e}")
    ...
    # Interactive hover: show channel/energy and counts on any plotted line/marker
    try:
        import mplcursors
        hover_lines = [ln for ln in ax.get_lines() if ln.get_label() and not ln.get_label().startswith('_')]
        if hover_lines:
            cursor = mplcursors.cursor(hover_lines, hover=2)
            ...
            def _on_add(sel):
                ...
                ch = sel.target[0]
                cts = sel.target[1]
                energy = cal_offset + cal_gain * ch
                text = (f"{label}\n"
                        f"Ch {ch:.2f} | {energy:.2f} keV\n"
                        f"{cts:.2f} cts")
                ...
                sel.annotation.get_bbox_patch().set_linewidth(1.5)
