...
# =============================================================================
# ELEMENT SPECTRUM PARSING (for simulation-guided A_A calculation)
# =============================================================================

def parse_element_spectrum(filepath: str) -> Dict:
    """
    ...
    # First column is 'channel', rest are element names
    if header[0].lower() != 'channel':
        raise ValueError(f"Expected 'channel' as first column, got: {header[0]}")
    
    element_names = header[1:]
    
    # Identify unique elements (not isotopes)
    # e.g., ['C', '12C', '13C', 'O', '16O', '17O', '18O', ...]
    # Elements without numbers are the summed totals
    unique_elements = []
    isotope_columns = {}  # element -> list of column indices
    ...
    for i, name in enumerate(element_names):
        ...
        if name and name[0].isalpha():
            ...
            isotope_columns[name] = [i]
        else:
            # This is an isotope (e.g., '12C', '16O')
            # Find the parent element by stripping leading numbers
            parent = ''.join(c for c in name if c.isalpha())
            if parent in isotope_columns:
                isotope_columns[parent].append(i)
    
    # Parse data lines
    channels = []
    data_columns = {name: [] for name in element_names}
    ...
    for name in element_names:
        element_spectra[name] = np.array(data_columns[name])
    
    # Calculate total simulated (sum of all element contributions)
    # Use only the main element columns (not isotopes) to avoid double counting
...
def calculate_element_counts_from_spectrum(
    ...
    # Channels might not start at 0, so we need to find the indices
    roi_mask = (channels >= roi_start) & (channels <= roi_end)
    
    if not np.any(roi_mask):
        return 0, 0.0, 0
    
    # Get simulated element spectrum in ROI
    element_spectra = element_spectrum_data['element_spectra']
    total_simulated = element_spectrum_data['total_simulated']
    
    # Find the element (try exact match first, then case-insensitive)
    sim_element = None
    for elem_name in element_spectra.keys():
        if elem_name == element:
            sim_element = element_spectra[elem_name]
            break
        elif elem_name.lower() == element.lower():
            sim_element = element_spectra[elem_name]
            break
    
    if sim_element is None:
        # Element not found in spectrum file
        return 0, 0.0, 0
    
    # Sum in ROI
    sim_element_roi = np.sum(sim_element[roi_mask])
    ...
    # Need to align channels - assume experimental spectrum starts at channel 0
...
def get_available_elements(element_spectrum_data: Dict) -> List[str]:
    ...
    Returns only the main elements (not individual isotopes).
    """
